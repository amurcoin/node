...
import com.amurcoin.transaction.lease.LeaseTransaction.Status.{Active, Canceled}
...
object LeaseStatusTestSuite {
  ...
  private val minerConfig = ConfigFactory.parseString(s"""amurcoin {
       |   miner{
       |      enable = yes
  ...
  private val notMinerConfig = ConfigFactory.parseString(s"""amurcoin {
       |   miner.enable = no
