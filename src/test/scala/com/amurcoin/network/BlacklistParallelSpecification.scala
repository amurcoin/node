...
import com.amurcoin.settings.{NetworkSettings, loadConfig}
...
class BlacklistParallelSpecification extends FeatureSpec with GivenWhenThen with ParallelTestExecution {
  ...
  private val config = loadConfig(ConfigFactory.parseString("""amurcoin.network {
      |  known-peers = []
      |  file = null
      |  black-list-residence-time: 1s
      |}""".stripMargin))

  private val networkSettings = config.as[NetworkSettings]("amurcoin.network")
