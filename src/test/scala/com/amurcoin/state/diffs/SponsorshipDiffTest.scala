...
class SponsorshipDiffTest extends PropSpec with PropertyChecks with Matchers with TransactionGen {
  ...
  property("cannot cancel sponsorship") {
    ...
    forAll(setup) {
      case (genesis, issueTx, sponsorTx, senderNotIssuer, insufficientFee) =>
        ...
        assertDiffEi(setupBlocks, block(Seq(insufficientFee)), s) { blockDiffEi =>
          blockDiffEi should produce("does not exceed minimal value of 100000000 AMURCOIN: 99999999")
        }
    }
  }
  ...
  property("sponsor has no AMURCOIN but receives them just in time") {
    val s = settings(0)
    val setup = for {
      master    <- accountGen
      recipient <- accountGen
      ...
      amurcoinTransfer = TransferTransactionV1
        ...
        .right
        .get
      backAmurcoinTransfer = TransferTransactionV1
        .selfSigned(None, recipient, master, 100000, ts + 5, Some(assetId), 100, Array.emptyByteArray)
        .right
        .get
    } yield (genesis, issue, sponsor, assetTransfer, amurcoinTransfer, backAmurcoinTransfer)

    forAll(setup) {
      case (genesis, issue, sponsor, assetTransfer, amurcoinTransfer, backAmurcoinTransfer) =>
        assertDiffAndState(Seq(block(Seq(genesis, issue, sponsor, assetTransfer, amurcoinTransfer))), block(Seq(backAmurcoinTransfer)), s) {
          case (diff, state) =>
