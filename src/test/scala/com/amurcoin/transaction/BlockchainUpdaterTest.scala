...
class BlockchainUpdaterTest extends FreeSpec with Matchers with HistoryTest with ShouldVerb with WithState {
  ...
  private val AmurcoinSettings = history.DefaultAmurcoinSettings.copy(
    blockchainSettings = history.DefaultAmurcoinSettings.blockchainSettings.copy(
      functionalitySettings = history.DefaultAmurcoinSettings.blockchainSettings.functionalitySettings.copy(
        featureCheckBlocksPeriod = ApprovalPeriod,
        blocksForFeatureActivation = (ApprovalPeriod * 0.9).toInt,
        preActivatedFeatures = Map.empty
      )
    ),
    featuresSettings = history.DefaultAmurcoinSettings.featuresSettings.copy(autoShutdownOnUnsupportedFeature = true)
  )

  private val AmurcoinSettingsWithDoubling = AmurcoinSettings.copy(
    blockchainSettings = AmurcoinSettings.blockchainSettings.copy(
      functionalitySettings = AmurcoinSettings.blockchainSettings.functionalitySettings.copy(
  ...
  "features approved and accepted as height grows" in withDomain(AmurcoinSettings) { domain =>
  ...
  "features rollback with block rollback" in withDomain(AmurcoinSettings) { domain =>
    val b = domain.blockchainUpdater
    b.processBlock(genesisBlock)
  ...
  "feature activation height is not overriden with further periods" in withDomain(AmurcoinSettings) { domain =>
    val b = domain.blockchainUpdater
  ...
  "feature activated only by 90% of blocks" in withDomain(AmurcoinSettings) { domain =>
    val b = domain.blockchainUpdater
  ...
  "features votes resets when voting window changes" in withDomain(AmurcoinSettings) { domain =>
    val b = domain.blockchainUpdater
  ...
  "sunny day test when known feature activated" in withDomain(AmurcoinSettings) { domain =>
    val b = domain.blockchainUpdater
    b.processBlock(genesisBlock)
  ...
  "empty blocks should not disable activation" in withDomain(AmurcoinSettings) { domain =>
    val b = domain.blockchainUpdater
  ...
  "doubling of feature periods works in the middle of activation period" in withDomain(AmurcoinSettingsWithDoubling) { domain =>
    val b = domain.blockchainUpdater
  ...
  "doubling of feature periods should work after defined height" in withDomain(AmurcoinSettingsWithDoubling) { domain =>
    val b = domain.blockchainUpdater
