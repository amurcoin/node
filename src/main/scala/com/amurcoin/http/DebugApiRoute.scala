...
case class DebugApiRoute(ws: WavesSettings,
  ...
  private lazy val amurcoinConfig: JsObject = Json.obj("amurcoin" -> (fullConfig \ "amurcoin").get)
  ...
  @ApiImplicitParams(
    Array(
      new ApiImplicitParam(
        ...
        required = true,
        paramType = "body",
        dataType = "com.amurcoin.http.DebugMessage",
        defaultValue = "{\n\t\"message\": \"foo\"\n}"
      )
    ))
  ...
  @ApiImplicitParams(
    Array(
      new ApiImplicitParam(
        ...
        required = true,
        paramType = "body",
        dataType = "com.amurcoin.http.RollbackParams",
  ...
  def configInfo: Route = (path("configInfo") & get & parameter('full.as[Boolean]) & withAuth) { full =>
    complete(if (full) fullConfig else amurcoinConfig)
  }
