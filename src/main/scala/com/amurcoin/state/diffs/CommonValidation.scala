...
import com.amurcoin.transaction.ValidationError._
...
object CommonValidation {
  ...
  def disallowSendingGreaterThanBalance[T <: Transaction](blockchain: Blockchain,
    ...
    if (blockTime >= settings.allowTemporaryNegativeUntil) {
      def checkTransfer(sender: Address, assetId: Option[AssetId], amount: Long, feeAssetId: Option[AssetId], feeAmount: Long) = {
        ...
        val oldAmurcoinBalance = blockchain.portfolio(sender).balance

        val newAmurcoinBalance = oldAmurcoinBalance + spendings.balance
        if (newAmurcoinBalance < 0) {
          Left(
            GenericError(
              "Attempt to transfer unavailable funds: Transaction application leads to " +
                s"negative amurcoin balance to (at least) temporary negative state, current balance equals $oldAmurcoinBalance, " +
                s"spends equals ${spendings.balance}, result is $newAmurcoinBalance"))
  ...
  def getMinFee(blockchain: Blockchain, fs: FunctionalitySettings, height: Int, tx: Transaction): Either[ValidationError, (Option[AssetId], Long)] = {
    ...
    feeAfterSponsorship(tx.assetFee._1)
      ...
      .map {
        case (Some((assetId, assetInfo)), amountInAmurcoin) => (Some(assetId), Sponsorship.fromAmurcoin(amountInAmurcoin, assetInfo.sponsorship))
        case (None, amountInAmurcoin)                       => (None, amountInAmurcoin)
      }
  }
  ...
  def checkFee(blockchain: Blockchain, fs: FunctionalitySettings, height: Int, tx: Transaction): Either[ValidationError, Unit] = {
      ...
      if (height < Sponsorship.sponsoredFeesSwitchHeight(blockchain, fs)) Right(inputFee)
        ...
        for {
          ...
          feeAmount <- feeAssetId match {
            ...
            case Some(x) =>
              for {
                ...
                amurcoinFee <- Either.cond(
                  assetInfo.sponsorship > 0,
                  Sponsorship.toAmurcoin(feeAmount, assetInfo.sponsorship),
          ...
          _ <- Either.cond(
            ...
            (),
            GenericError(
              s"Fee in ${feeAssetId.fold("AMURCOIN")(_.toString)} for ${tx.builder.classTag} does not exceed minimal value of $minimumFee AMURCOIN: $feeAmount")
          )
    ...
    def restFeeAfterSmartTokens(inputFee: (Option[AssetId], Long)): Either[ValidationError, (Option[AssetId], Long)] =
      if (isSmartToken) {
        val (feeAssetId, feeAmount) = inputFee
        for {
          _ <- Either.cond(feeAssetId.isEmpty, (), GenericError("Transactions with smart tokens require Amurcoin as fee"))
    ...
    def restFeeAfterSmartAccounts(inputFee: (Option[AssetId], Long)): Either[ValidationError, (Option[AssetId], Long)] =
      if (hasSmartAccountScript) {
        val (feeAssetId, feeAmount) = inputFee
        for {
          _ <- Either.cond(feeAssetId.isEmpty, (), GenericError("Transactions from scripted accounts require Amurcoin as fee"))
          restFeeAmount = feeAmount - ScriptExtraFee
          _ <- Either.cond(
            restFeeAmount >= 0,
            (),
